...
|---|---|---|
| `tests/benchmarks/test_retrieval_benchmark.py` | 5 | Recall@10 across 100 tasks in 10 orthonormal groups. Uses `_ControlledEmbedder` (lookup-table bypass of DANN) and `_SimpleFaissIndex` (exact cosine without a FAISS binary). Asserts mean recall > 0.85, per-group recall > 0.85, ≥8 same-group results in top-10, zero cross-group results above similarity threshold 0.50, and empty-index edge case. |
